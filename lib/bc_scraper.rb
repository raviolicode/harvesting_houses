...
class BuscandoCasaScraper < Scraper
  ...
  def search_houses 
    ...
    document = get_nokogiri_document_from_url 'buscando_casa_search_properties', search_conditions 

    document.css('input').map{|e| e.attributes['value'].value}
  end

  def scrap_house reference_id
    document = get_nokogiri_document_from_url 'buscando_casa_house_details', reference_id
